...
compatibility:
  dependencies:
    - ARTCLAW REST API (http://staging.artclaw.com/gen-api/v1)
metadata:
  {
    "openclaw":
      {
...
**API 基地址：** `http://staging.artclaw.com/gen-api/v1`
...
```bash
curl -X POST http://staging.artclaw.com/gen-api/v1/auth/verify \
...
| 类别 | 接口 | 方法 | 说明 | 需要 Key |
|------|------|------|------|----------|
| **生图** | `/gen-api/v1/generate/image` | POST | 文生图 / 图生图（支持参考图） | ✅ |
| **生图** | `/gen-api/v1/generate/marketing-image` | POST | 营销广告图（自动增强提示词） | ✅ |
| **生图** | `/gen-api/v1/generate/product-carousel` | POST | 产品套图（多图轮播，自动风格匹配） | ✅ |
| **生视频** | `/gen-api/v1/generate/video` | POST | 文生视频 / 图生视频（I2V） | ✅ |
| **PPT** | `/gen-api/v1/generate/ppt` | POST | AI PPT 幻灯片生成（多页图片） | ✅ |
| **分析** | `/gen-api/v1/analyze/image` | POST | AI 图片理解与描述 | ✅ |
| **分析** | `/gen-api/v1/analyze/video` | POST | AI 视频内容分析 | ✅ |
| **分析** | `/gen-api/v1/analyze/script` | POST | 短剧剧本分析 + 互动节点设计 | ✅ |
| **分析** | `/gen-api/v1/analyze/characters` | POST | 从故事文本生成人物小传 | ✅ |
| **工作流** | `/gen-api/v1/workflows` | GET | 列出所有可用预设工作流 | ✅ |
| **工作流** | `/gen-api/v1/workflows/{id}/run` | POST | 执行预设工作流 | ✅ |
| **任务管理** | `/gen-api/v1/jobs/{job_id}` | GET | 查询任务状态和结果 | ✅ |
| **任务管理** | `/gen-api/v1/jobs` | GET | 列出历史任务 | ✅ |
| **任务管理** | `/gen-api/v1/jobs/{job_id}/cancel` | POST | 取消正在进行的任务 | ✅ |
| **账户** | `/gen-api/v1/account/info` | GET | 查询账户余额与用量 | ✅ |
| **鉴权** | `/gen-api/v1/auth/verify` | POST | 验证 API Key 有效性 | ❌ |
| **Prompt** | `/gen-api/v1/prompts/logo` | POST | 生成 Logo 提示词（免费） | ❌ |
| **Prompt** | `/gen-api/v1/prompts/cover` | POST | 生成封面图提示词（免费） | ❌ |
| **Prompt** | `/gen-api/v1/prompts/marketing` | POST | 增强营销图提示词（免费） | ❌ |
| **Prompt** | `/gen-api/v1/prompts/carousel` | POST | 生成轮播图提示词系列（免费） | ❌ |

---

## 鉴权方式

除 Prompt 工具（`/gen-api/v1/prompts/*`）和 Key 验证（`/gen-api/v1/auth/verify`）外，所有接口均需在 HTTP Header 中传入 API Key：
...
| 分类 | 路径 | 限制 |
|------|------|------|
| 生成类 | `/gen-api/v1/generate/*`、`/gen-api/v1/workflows/*`、`/gen-api/v1/analyze/*` | 10 次/分钟 |
| 查询类 | `/gen-api/v1/jobs/*`、`/gen-api/v1/account/*` | 60 次/分钟 |
| 公开类 | `/gen-api/v1/prompts/*` | 30 次/分钟 |
...
3. 以一定间隔 GET /gen-api/v1/jobs/{job_id} 检查状态。任务可能在队列中排队，不要因为 pending 持续很久就认为出错
...
BASE="http://staging.artclaw.com/gen-api/v1"
...
### POST `/gen-api/v1/generate/image` — 文生图 / 图生图
...
```bash
curl -X POST http://staging.artclaw.com/gen-api/v1/generate/image \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: vk_xxx" \
  -d '{
...
### POST `/gen-api/v1/generate/video` — 文生视频 / 图生视频

根据文本描述生成视频；若传入 `content_items` 参考素材则自动进入图生视频（I2V）模式。
...
| `duration` | int | `5` | 时长（秒），实际范围取决于模型（通常 4–15） |
| `resolution` | string | `"1K"` | `480p` / `720p` / `1080p` / `1K` / `2K` |
| `model` | string | `"doubao-seedance-2-0-fast-260128"` | 指定模型 ID（可选） |
| `provider` | string | `""` | 模型提供商，留空时根据 model 自动推断。可选：`seedance` / `gemini` / `vidu` / `grok` / `sora` 等 |
| `content_items` | list[object] | `[]` | 参考素材列表，每项含 `role` + `url`（见下方说明）。传入后自动切为图生视频模式 |
| `seed` | int \| null | `null` | 随机种子（可选） |
| `generate_audio` | bool | `true` | 是否生成音频 |
| `callback_url` | string | `""` | 回调地址 |

**`content_items` 角色说明：**

每个 content_item 是一个对象，包含 `role`（角色）和 `url`（资源 URL）两个字段：

| role | 说明 |
|------|------|
| `first_frame` | 首帧图片（I2V 起始帧） |
| `last_frame` | 尾帧图片（I2V 结束帧） |
| `reference_image` | 参考图片（风格 / 角色参考） |
| `reference_video` | 参考视频 |
| `reference_audio` | 参考音频 |

> **注意：** 不同模型支持的 role 不同。`first_frame` / `last_frame` 被大多数模型支持；`reference_image` / `reference_video` / `reference_audio` 主要用于 Seedance 2.0 系列模型，而且一旦有任意参考（`reference_xxx`），`first_frame`和`last_frame`都不再支持，可以理解为，有三种视频生成模式：首帧模式、首尾帧模式和全能参考模式。

**示例（文生视频）：**

```bash
curl -X POST http://staging.artclaw.com/gen-api/v1/generate/video \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: vk_xxx" \
...
curl -X POST http://staging.artclaw.com/gen-api/v1/generate/video \
  ...
  -d '{
    "prompt": "人物缓慢回头，夕阳余晖",
    "content_items": [
      {"role": "first_frame", "url": "https://example.com/portrait.jpg"}
    ]
  }'
```
...
curl -X POST http://staging.artclaw.com/gen-api/v1/generate/video \
  ...
  -d '{
    ...
    "duration": 8,
    "content_items": [
      {"role": "first_frame", "url": "https://example.com/wide_shot.jpg"},
      {"role": "last_frame", "url": "https://example.com/close_up.jpg"}
    ]
  }'
```
...
### POST `/gen-api/v1/generate/marketing-image` — 营销广告图
...
### POST `/gen-api/v1/generate/product-carousel` — 产品套图
...
### POST `/gen-api/v1/generate/ppt` — AI PPT 生成
...
### GET `/gen-api/v1/workflows` — 列出可用工作流
...
### POST `/gen-api/v1/workflows/{workflow_id}/run` — 执行工作流
...
```bash
curl -X POST http://staging.artclaw.com/gen-api/v1/workflows/2603-wf-anime-production/run \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: vk_xxx" \
...
### POST `/gen-api/v1/analyze/image` — AI 图片理解
...
### POST `/gen-api/v1/analyze/video` — AI 视频分析
...
### POST `/gen-api/v1/analyze/script` — 短剧剧本分析
...
### POST `/gen-api/v1/analyze/characters` — 人物小传生成
...
**GET `/gen-api/v1/jobs/{job_id}`** — 查询任务状态（含进度和结果）
...
**POST `/gen-api/v1/jobs/{job_id}/cancel`** — 取消尚未完成的任务

---

**GET `/gen-api/v1/account/info`** — 查询账户余额与用量

---

### Prompt 增强接口（免费，无需 API Key）

**POST `/gen-api/v1/prompts/logo`** — 生成专业 Logo 提示词
...
**POST `/gen-api/v1/prompts/cover`** — 生成封面图提示词
...
**POST `/gen-api/v1/prompts/marketing`** — 智能增强营销提示词
...
**POST `/gen-api/v1/prompts/carousel`** — 生成整套轮播图提示词
...
5. **支持并发** — 可同时提交多个任务，用 `GET /gen-api/v1/jobs` 统一跟踪
...
| `404` | `Workflow not found` | workflow_id 不存在 | 先调用 `GET /gen-api/v1/workflows` 确认可用 ID |
