...
compatibility:
  dependencies:
    - ARTCLAW REST API (http://staging.artclaw.com/api/v1)
metadata:
  {
    "openclaw":
      {
...
**API 基地址：** `http://staging.artclaw.com/api/v1`
...
```bash
curl -L -X POST http://staging.artclaw.com/api/v1/auth/verify \
...
BASE="http://staging.artclaw.com/api/v1"
...
```bash
curl -L -X POST http://staging.artclaw.com/api/v1/generate/image \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: vk_xxx" \
  -d '{
...
```bash
curl  -L -X POST http://staging.artclaw.com/api/v1/generate/video \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: vk_xxx" \
...
```bash
curl -X POST http://staging.artclaw.com/api/v1/generate/video \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: vk_xxx" \
  -d '{
    "prompt": "镜头从远景推进到人物特写",
