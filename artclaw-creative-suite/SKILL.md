...
compatibility:
  dependencies:
    - ARTCLAW REST API (http://artclaw.com/api/v1)
metadata:
  {
    "openclaw":
      {
...
**API 基地址：** `http://artclaw.com/api/v1`

> **⚠️ 重定向提示： 使用 `curl` 时**必须加 `-L`** 参数以自动跟随重定向，否则会收到空响应。本文档所有 curl 示例均已包含 `-L`。如使用 Python `httpx` / `requests`，默认自动跟随重定向，无需额外处理。
...
```bash
curl -L -X POST http://artclaw.com/api/v1/auth/verify \
...
**所有生成类接口**（图片/视频/工作流）均为异步模式。调用后返回 `job_id`，需要由client轮询状态直到完成。
...
```bash
curl -L -X POST http://artclaw.com/api/v1/generate/image \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: vk_xxx" \
  -d '{
...
```bash
curl  -L -X POST http://artclaw.com/api/v1/generate/video \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: vk_xxx" \
...
```bash
curl -X POST http://artclaw.com/api/v1/generate/video \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: vk_xxx" \
  -d '{
    "prompt": "镜头从远景推进到人物特写",
...
{
  "workflows": [
    {"name": "AI 动画番剧生成", "description": "..."},
    {"name": "AI 微电影生成", "description": "..."},
  ],
  "total": 2
}
```
...
**示例：**
```bash
curl -L -X POST http://artclaw.com/api/v1/workflows/anime-production/run \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: vk_xxx" \
...
`metadata` 中的字段（按任务类型不同）：
- 工作流：`execution_id`、`workflow_name`（**注意：** 当前不返回步骤级进度信息，轮询时无法获知"第几步/共几步"）
- 失败：`error_detail`
...
| `type` | string | `""` | 过滤类型：`image_generation` / `video_generation` / `marketing_image` / `workflow` |
