...
```bash
curl -X POST http://staging.artclaw.com/api/v1/auth/verify \
...
```bash
curl -X POST http://staging.artclaw.com/api/v1/generate/image \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: vk_xxx" \
  -d '{
...
```bash
curl -X POST http://staging.artclaw.com/api/v1/workflows/2603-wf-anime-production/run \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: vk_xxx" \
